...
\end{equation}
就能在当前状态保持相同热流。逐时自由变化的 $h(t)$ 和 $\varepsilon(t)$ 因而没有唯一分解。该结论限定于本文的均匀灰体边界、已知表面温度和已知炉温。若参数在一段时间内保持常数，变化的 $h_{\mathrm{rad}}$ 可提供额外历史信息。对本文逐时变化的边界，$\Heff$ 是当前热流和单步状态转移的充分输入。

...
主模型采用三层残差多层感知机，每层 128 个隐藏单元。输入和输出均依据训练集统计量标准化。当前固定网格只有 41 个节点，MLP 可以直接处理完整状态，并适合在控制器内批量评价候选动作。卷积网络和神经算子更适合后续的高分辨率或跨几何扩展。
...
\begin{equation}
  ...
  \quad j=0,\ldots,K-1,
\end{equation}
其中 $\widehat{\bm{s}}_n=\bm{s}_n$，本文取 $K=5$。多步数据损失为
\begin{equation}
  \mathcal{L}_{\mathrm{data}}
  =\frac{1}{KN}\sum_{j=1}^{K}\left\|
  \widehat{\bm{s}}_{n+j}-\bm{s}_{n+j}
  \right\|_2^2.
  \label{eq:data_loss}
\end{equation}
...
内部节点物理残差由式\eqref{eq:discrete_balance}计算。对预测状态 $\widehat{\bm{s}}_{n+1}$，定义
\begin{equation}
  r_i=\rho c_{p,i}^{n}\frac{\widehat{T}_i^{n+1}-T_i^n}{\Delta t}
  -\frac{1}{\Delta x}\left(
  k_{i+1/2}^{n}\frac{\widehat{T}_{i+1}^{n+1}-\widehat{T}_{i}^{n+1}}{\Delta x}
  -k_{i-1/2}^{n}\frac{\widehat{T}_{i}^{n+1}-\widehat{T}_{i-1}^{n+1}}{\Delta x}
  \right).
\end{equation}
表面节点残差加入式\eqref{eq:mixed_boundary}的净热流。为避免量纲和幅值控制梯度，残差按训练集温度增量尺度归一化。多步物理损失写为
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:total_loss}
\end{equation}
权重扫描包含 $0$、$10^{-3}$、$10^{-2}$ 和 $10^{-1}$。三随机种子验证均值选择 $\lambda_{\mathrm{phy}}=10^{-3}$ 作为正式设置。
...
\end{equation}
该矩阵的秩为 1。温度响应只能确定一个标量组合。任取扰动 $\delta\varepsilon$，只要
\begin{equation}
  \delta h=-h_{\mathrm{rad}}\delta\varepsilon,
\end{equation}
就能在当前状态保持相同热流。逐时自由变化的 $h(t)$ 和 $\varepsilon(t)$ 因而没有唯一分解。$\Heff$ 是当前边界转移的充分输入。
...
\begin{equation}
  ...
  \label{eq:heff_model}
\end{equation}
该表示消除了训练输入中的等价方向。网络无需学习同一边界热流对应的多组参数组合。对反事实动作 $a'$，$h$ 与 $\varepsilon$ 的分解仍决定新的辐射贡献。因此，闭环规划阶段使用增广状态滤波器保持二者的后验集合，并由每个集合成员计算候选动作下的 $\Heff(a')$。
...
所有正式结果至少使用三个训练种子。参数比较在相同测试轨迹上进行。模型选择只访问验证集。BDF 参考集和控制环境不参与训练。
